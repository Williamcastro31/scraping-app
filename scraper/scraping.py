            ...
            with sync_playwright() as p:
                ...
                if nenhum_resultado_locator.count() > 0:
                    ...
                    if texto.strip() == "Nenhum resultado encontrado.":
                        browser.close()
                        return {'erro_cliente': f"Cliente não encontrado para o CNPJ informado. ({self.cnpj})"}
                else:
                ...
                try:
                    ...
                    if elemento_produto.count() == 0:
                        browser.close()
                        return {'erro_produto': f"Produto '{self.descricao}' não encontrado no cadastro do cliente."}
                    elemento_produto.first.click()
                except Exception as e:
                    browser.close()
                    return {'erro_produto': f"Produto '{self.descricao}' não encontrado no cadastro do cliente. erro: {e}"}
...
#teste = Scraping_Cotacao("elfa1", "Mudar@2024", "7897572020115", "KEYTRUDA 17%", "00132229000341", "8383393")
#teste.trade_fidelize()

#teste2 = Scraping_Cotacao("elfa1", "Mudar@2024", "7897572020115", "KEYTRUDA", "00382069000127", "8383393")
#teste2.trade_fidelize()